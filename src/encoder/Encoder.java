...
public class Encoder implements Executor {
  ...
  public void setAdapter(Executor provider, Object adapter, AdapterType typeOfAdapter) {
    switch (typeOfAdapter) {
      case DOUBLE: {
        ((DoubleAdapterClass) adapter).setMetrics(start, blockSize);
        break;
      }
      case BYTE: {
        ((ByteAdapterClass) adapter).setMetrics(start, blockSize);
        break;
      }
    }
    ...
    try {
      while (!Thread.interrupted()) {
        ...
        for (Integer st : starts) {
          ...
          switch (ad.getValue()) {
            case DOUBLE: {
              DoubleAdapterClass doubleAdapter = (DoubleAdapterClass) ad.getKey();
              Double cur;
            ...
            case BYTE: {
              ByteAdapterClass byteAdapter = (ByteAdapterClass) ad.getKey();
              Byte cur;
  ...
  private void sortAdapters() {
    for (Pair<Object, AdapterType> ad : adapters.values()) {
      switch (ad.getValue()) {
        case DOUBLE: {
          Integer st = ((DoubleAdapterClass)ad.getKey()).getStart();
          starts.add(st);
          startToAdapter.put(st, ad);
          break;
        }
        case BYTE: {
          Integer st = ((ByteAdapterClass)ad.getKey()).getStart();
          starts.add(st);
          startToAdapter.put(st, ad);
          break;
        }
